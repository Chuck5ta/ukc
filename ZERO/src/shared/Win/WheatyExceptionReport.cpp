...
inline LPTSTR ErrorMessage(DWORD dw)
{
    ...
    FormatMessage(
        ...
        0, nullptr);
    return static_cast<LPTSTR>(lpMsgBuf);
}
...
BOOL WheatyExceptionReport::_GetProcessorName(TCHAR* sProcessorName, DWORD maxcount)
{
    ...
    lRet = ::RegQueryValueEx(hKey, _T("ProcessorNameString"), nullptr, nullptr,
                             reinterpret_cast<LPBYTE>(szTmp), &cntBytes);
    if (lRet != ERROR_SUCCESS)
        { return FALSE; }
    ::RegCloseKey(hKey);
    sProcessorName[0] = '\0';
...
BOOL WheatyExceptionReport::_GetWindowsVersion(TCHAR* szVersion, DWORD cntMax)
{
    ...
    bOsVersionInfoEx = ::GetVersionEx(reinterpret_cast<LPOSVERSIONINFO>(&osvi));
    if (!bOsVersionInfoEx)
    {
...
//    #ifdef _M_IX86                                        // X86 Only!
    ...
    SymEnumSymbols(GetCurrentProcess(),
                   reinterpret_cast<DWORD64>(GetModuleHandle(szFaultingModule)),
    ...
    PVOID addr, PTSTR szModule, DWORD len, DWORD& section, DWORD_PTR& offset)
{
    ...
    if (!GetModuleFileName(reinterpret_cast<HMODULE>(hMod), szModule, len))
        { return FALSE; }
    ...
    PIMAGE_NT_HEADERS pNtHdr = reinterpret_cast<PIMAGE_NT_HEADERS>(hMod + DWORD_PTR(pDosHdr->e_lfanew));

    PIMAGE_SECTION_HEADER pSection = IMAGE_FIRST_SECTION(pNtHdr);

    DWORD_PTR rva = reinterpret_cast<DWORD_PTR>(addr) - hMod;                 // RVA is offset from module load address
    ...
    bool bWriteVariables, HANDLE pThreadHandle)             // true if local/params should be output
{
    ...
    while (true)
    {
        ...
        else                                                // No symbol found.  Print out the logical address instead.
        {
            ...
            GetLogicalAddress(reinterpret_cast<PVOID>(sf.AddrPC.Offset),
    ...
    PVOID         UserContext)
{
    ...
    __try
    {
        if (FormatSymbolValue(pSymInfo, static_cast<STACKFRAME*>(UserContext),
...
//    char* pszCurrBuffer = pszBuffer;
    ...
    DWORD_PTR pVariable;                                // Will point to the variable's data in memory
    ...
    {
        pVariable = static_cast<DWORD_PTR>(pSym->Address);               // It must be a global variable
    }
    ...
    if (!bHandled)
    {
        ...
        //                                  reinterpret_cast<PVOID>(pVariable));
    }
    ...
    for (unsigned i = 0; i < dwChildrenCount; ++i)
    {
        ...
        if (!bHandled2)
        {
            ...
            pszCurrBuffer = FormatOutputValue(pszCurrBuffer, basicType,
                                              length, reinterpret_cast<PVOID>(dwFinalOffset));
        ...
        PVOID pAddress)
{
    ...
    if (length == 1)
        { pszCurrBuffer += sprintf(pszCurrBuffer, " = %X", *static_cast<PBYTE>(pAddress)); }
    else if (length == 2)
        { pszCurrBuffer += sprintf(pszCurrBuffer, " = %X", *static_cast<PWORD>(pAddress)); }
    else if (length == 4)
    {
        if (basicType == btFloat)
        {
            pszCurrBuffer += sprintf(pszCurrBuffer, " = %f", *static_cast<PFLOAT>(pAddress));
        }
        else if (basicType == btChar)
        {
            if (!IsBadStringPtr(*static_cast<PSTR*>(pAddress), 32))
            {
                pszCurrBuffer += sprintf(pszCurrBuffer, " = \"%.31lu\"",
                                         *static_cast<PDWORD>(pAddress));
            }
            else
            ...
            { pszCurrBuffer += sprintf(pszCurrBuffer, " = %X", *static_cast<PDWORD>(pAddress)); }
    }
    else if (length == 8)
    {
        if (basicType == btFloat)
        {
            pszCurrBuffer += sprintf(pszCurrBuffer, " = %lf",
                                     *static_cast<double*>(pAddress));
        }
        else
            pszCurrBuffer += sprintf(pszCurrBuffer, " = %I64X",
                                     *static_cast<DWORD64*>(pAddress));
    }
