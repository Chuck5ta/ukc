...
DBCFileLoader::DBCFileLoader(): recordSize(0), recordCount(0), fieldCount(0), stringSize(0), stringTable(nullptr)
{
    data = nullptr;
    ...
    */
    ...
    for (uint32 y = 0; y < recordCount; ++y)
    {
        ...
        for (uint32 x = 0; x < fieldCount; ++x)
        {
            ...
            {
                case DBC_FF_FLOAT:
                    *reinterpret_cast<float*>(&dataTable[offset]) = getRecord(y).getFloat(x);
                    offset += sizeof(float);
                    break;
                case DBC_FF_IND:
                case DBC_FF_INT:
                    *reinterpret_cast<uint32*>(&dataTable[offset]) = getRecord(y).getUInt(x);
                    offset += sizeof(uint32);
                    break;
                case DBC_FF_BYTE:
                    *reinterpret_cast<uint8*>(&dataTable[offset]) = getRecord(y).getUInt8(x);
                    offset += sizeof(uint8);
                    break;
                case DBC_FF_STRING:
                    *reinterpret_cast<char**>(&dataTable[offset]) = nullptr; // will replace non-empty or "" strings in AutoProduceStrings
                    offset += sizeof(char*);
                    break;
                case DBC_FF_LOGIC:
                    assert(false && "Attempted to load DBC files that do not have field types that match what is in the core. Check DBCfmt.h or your DBC files.");
            ...
            switch (format[x])
            {
                ...
                case DBC_FF_STRING:
                {
                    ...
                    if (!*slot || !** slot)
                    {
                        ...
                        *slot = stringPool + (st - reinterpret_cast<const char*>(stringTable));
                    }
                    offset += sizeof(char*);
                    break;
                }
