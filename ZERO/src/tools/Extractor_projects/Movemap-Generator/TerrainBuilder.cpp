    ...
    void TerrainBuilder::getLoopVars(Spot portion, int& loopStart, int& loopEnd, int& loopInc)
    {
        switch (portion)
        {
            ...
            case BOTTOM:
                ...
                loopInc = 1;
                break;
            default: // Make default perform the same as entire
                loopStart = 0;
                loopEnd = V8_SIZE_SQ;
                loopInc = 1;
                break;
    ...
    void TerrainBuilder::getHeightCoord(int index, Grid grid, float xOffset, float yOffset, float* coord, float* v)
    {
        ...
        switch (grid)
        {
            ...
            case GRID_V8:
                ...
                coord[2] = v[index];
                break;
            default: // make default perform the same as GRID_V8
                coord[0] = (xOffset + index % (V8_SIZE) * GRID_PART_SIZE + GRID_PART_SIZE / 2.f) * -1.f;
                coord[1] = (yOffset + static_cast<int>(index / (V8_SIZE)) * GRID_PART_SIZE + GRID_PART_SIZE / 2.f) * -1.f;
                coord[2] = v[index];
                break;
                    ...
                    if (liquid)
                    {
                        ...
                        switch (liquid->GetType())
                        {
                            ...
                            case 3:
                                type = NAV_SLIME;
                                break;
                            default: // Make default use NAV_WATER
                                type = NAV_WATER;
                                break;
                        ...
                        for (uint32 i2 = 0; i2 < liqVerts.size(); ++i2)
                            { meshData.liquidVerts.append(liqVerts[i2].y, liqVerts[i2].z, liqVerts[i2].x); }

                        for (uint32 i3 = 0; i3 < liqTris.size() / 3; ++i3)
                        {
                            meshData.liquidTris.append(liqTris[i3 * 3 + 1] + liqOffset, liqTris[i3 * 3 + 2] + liqOffset, liqTris[i3 * 3] + liqOffset);
    ...
    void TerrainBuilder::loadOffMeshConnections(uint32 mapID, uint32 tileX, uint32 tileY, MeshData& meshData, const char* offMeshFilePath)
    {
        ...
        while (fgets(buf, 512, fp))
        {
            ...
            if ((mapID == mid) && (tileX == tx) && (tileY == ty))
            {
                ...
                meshData.offMeshConnectionsAreas.append(static_cast<unsigned char>(0xFF));
                meshData.offMeshConnectionsFlags.append(static_cast<unsigned short>(0xFF));  // all movement masks can make this path
            }
