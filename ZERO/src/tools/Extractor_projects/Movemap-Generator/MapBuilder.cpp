...
namespace MMAP
{
    MapBuilder::MapBuilder(char const* magic, float maxWalkableAngle, bool skipLiquid,
        ...
        m_magic(magic),
        m_numThreads(-1), m_threadPool(nullptr), m_poolActivated(false)
    {
    ...
    void MapBuilder::buildAllMaps()
    {
        ...
        if (activated())
        {
            Tile_Message_Block *finish_mb = new Tile_Message_Block(nullptr);
    ...
    void MapBuilder::buildMap(int mapID, bool standAlone)
    {
        ...
        dtNavMeshParams* meshParams = nullptr;
        ...
        printf(" %s map %03u [%u tiles]\n", activated() ? "Scheduling" : "Building", mapID, static_cast<unsigned int>(tiles->size()));
                                      ...
                                      dtNavMesh* navMesh)
    {
        ...
        if (m_debugOutput)
        {
            ...
            {
                unsigned short* v = &iv.polyMesh->verts[i * 3];
                v[0] += static_cast<unsigned short>(config.borderSize);
                v[2] += static_cast<unsigned short>(config.borderSize);
            }
