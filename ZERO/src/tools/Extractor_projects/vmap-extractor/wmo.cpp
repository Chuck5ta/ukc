...
WMORoot::WMORoot(std::string& filename) : nTextures(0), nGroups(0), nP(0), nLights(0), nModels(0), nDoodads(0), nDoodadSets(0), RootWMOID(0), liquidType(0), col(0), filename(filename)
{
}
...
WMOGroup::WMOGroup(std::string& filename) : groupName(0), descGroupName(0), mogpFlags(0), moprIdx(0), moprNItems(0), nBatchA(0), nBatchB(0), nBatchC(0), fogIdx(0), liquidType(0), groupWMOID(0), mopy_size(0), moba_size(0), LiquEx_size(0), nVertices(0), nTriangles(0), filename(filename),
                                            MOPY(nullptr), MOVI(nullptr), MoviEx(nullptr), MOVT(nullptr), MOBA(nullptr), MobaEx(nullptr), hlq(nullptr), LiquEx(nullptr), LiquBytes(nullptr), liquflags(0)
{
}
...
int WMOGroup::ConvertToVMAPGroupWmo(FILE* output, WMORoot* rootWMO, bool pPreciseVectorData, int iCoreNumber)
{
    ...
    if (LiquEx_size != 0)
    {
        ...
        {
            switch ((static_cast<uint8>(liquidEntry) - 1) & 3)
            {
                case 0:
...
WMOInstance::WMOInstance(MPQFile& f, std::string& WmoInstName, uint32 mapID, uint32 tileX, uint32 tileY, FILE* pDirfile): currx(0), curry(0), wmo(nullptr), indx(0), d3(0), doodadset(0)
{
    pos = Vec3D(0, 0, 0);
...
bool ExtractSingleWmo(std::string& fname, int iCoreNumber, const void *szRawVMAPMagic)
{
    ...
    {
        char cpy[4];
        strncpy(static_cast<char*>(cpy), rchr, 4);
        for (int i = 0; i < 4; ++i)
        {
            int m = cpy[i];
            if (isdigit(m))
