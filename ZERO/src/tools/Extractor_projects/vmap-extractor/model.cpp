...
Model::Model(std::string& filename) : boundingVertices(nullptr), filename(filename), vertices(nullptr), indices(nullptr), nIndices(0), ok(false)
{
}
...
bool Model::open(StringSet& failedPaths, int iCoreNumber)
{
    ...
    {
        boundingVertices = reinterpret_cast<ModelBoundingVertex*>(f.getBuffer() + uofsBoundingVertices);
        ...
        uint16* triangles = reinterpret_cast<uint16*>(f.getBuffer() + uofsBoundingTriangles);
...
bool Model::ConvertToVMAPModel(std::string& outfilename,int iCoreNumber, const void *szRawVMAPMagic)
{
    ...
    uint32 nIndexes = static_cast<uint32>(nIndices);
...
ModelInstance::ModelInstance(MPQFile& f, string& ModelInstName, uint32 mapID, uint32 tileX, uint32 tileY, FILE* pDirfile, int coreNumber): model(nullptr), d1(0), w(0)
{
    float ff[3];
    f.read(&id, 4);
    f.read(ff, 12);
